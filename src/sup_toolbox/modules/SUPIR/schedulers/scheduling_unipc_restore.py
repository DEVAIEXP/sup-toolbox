...
@dataclass
class UniPCRestoreSchedulerOutput(SchedulerOutput):
    prev_sample: torch.Tensor
    pred_original_sample: Optional[torch.Tensor] = None
    guided_pred_original_sample: Optional[torch.Tensor] = None


class UniPCRestoreScheduler(SchedulerMixin, ConfigMixin):
    """
    `UniPCMultistepScheduler` adapted for SUPIR restoration guidance.
    Copies original UniPC logic and minimally modifies the step function.
    Assumes UNet predicts epsilon and operates with integer timesteps.
    Uses no-op scale_model_input.
    ...
    Args:
        ...
        use_flow_sigmas (`bool`, *optional*, defaults to `False`):
            Whether to use flow sigmas for step sizes in the noise schedule during the sampling process (from flow matching models).
        ...
        sigma_data (`float`, *optional*, defaults to 0.5):
            The standard deviation of the data distribution from the EDM paper. Not directly used by UniPC's core
            update rule if `prediction_type` is `epsilon` and `predict_x0` is `True`, but kept for potential
    ...
    def step(
    ...
    ) -> Union[UniPCRestoreSchedulerOutput, Tuple]:
        """
        Predict the sample at the previous timestep by reversing the SDE using the UniPC (Unified Predictor-Corrector)
        algorithm. This function propagates the sample with the multistep UniPC and incorporates restoration guidance.

        Args:
            model_output (`torch.Tensor`):
                The direct output from the learned diffusion model (usually predicted noise, epsilon).
            timestep (`int` or `torch.Tensor`):
                The current discrete timestep in the diffusion chain (0-999).
            sample (`torch.Tensor`):
            ...
            z_center (`torch.Tensor`, *optional*):
                ...
                Defaults to `None`, in which case restoration guidance is not applied if `restoration_scale > 0`.
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`UniPCRestoreSchedulerOutput`] or `tuple`.

        Returns:
            [`UniPCRestoreSchedulerOutput`] or `tuple`:
                If `return_dict` is `True`, [`UniPCRestoreSchedulerOutput`] is returned, otherwise a `tuple`
        ...
        if self.config.predict_x0:
            model_output_for_history = guided_pred_original_sample.to(model_output.dtype)
        else:
            epsilon_guided = (sample.to(torch.float32) - alpha_t * guided_pred_original_sample) / sigma_t.clamp(min=1e-9)
            model_output_for_history = epsilon_guided.to(model_output.dtype)

        use_corrector = self.step_index > 0 and self.step_index - 1 not in self.config.disable_corrector and self.last_sample is not None
        sample_corrected = sample

        if use_corrector:
            sample_corrected = self.multistep_uni_c_bh_update(
        ...
        self.model_outputs[-1] = model_output_for_history
        ...
        self.last_sample = sample_corrected
        prev_sample = self.multistep_uni_p_bh_update(
            model_output=model_output,
            prev_timestep=self.timestep_list[0] if self.lower_order_nums > 0 else None,
        ...
        return UniPCRestoreSchedulerOutput(
