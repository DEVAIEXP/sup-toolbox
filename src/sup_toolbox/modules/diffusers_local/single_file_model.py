# This single_file_model file is a derivative work.
...
from diffusers.utils import deprecate, is_accelerate_available, is_torch_version, logging
...
def _get_single_file_loadable_mapping_class(cls):
    # changed here to support custom classes #elismasilva
...
class FromOriginalModelMixin:
    ...
    def from_single_file(cls, pretrained_model_link_or_path_or_dict: Optional[str] = None, **kwargs) -> Self:
        ...
        if mapping_class_name is None:
            raise ValueError(f"FromOriginalModelMixin is currently only compatible with {', '.join(SINGLE_FILE_LOADABLE_CLASSES.keys())}")
        ...
        if config is not None and original_config is not None:
            raise ValueError("`from_single_file` cannot accept both `config` and `original_config` arguments. Please provide only one of these arguments")
        ...
        if original_config is not None:
            ...
            if config is not None:
                if isinstance(config, str):
                    ...
                    raise ValueError(("Invalid `config` argument. Please provide a string representing a repo idor path to a local Diffusers model repo."))
                ...
                subfolder = subfolder or config.pop("subfolder", None)  # some configs contain a subfolder key, e.g. StableCascadeUNet
        ...
        if not diffusers_format_checkpoint:
            raise SingleFileComponentError(f"Failed to load {mapping_class_name}. Weights for this component appear to be missing in the checkpoint.")
        ...
        if low_cpu_mem_usage:
            ...
            unexpected_keys = [param_name for param_name in diffusers_format_checkpoint if param_name not in empty_state_dict]
        ...
        if len(unexpected_keys) > 0:
            logger.warning(f"Some weights of the model checkpoint were not used when initializing {cls.__name__}: \n {[', '.join(unexpected_keys)]}")
